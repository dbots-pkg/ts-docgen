...
var mainPromises = [];
if (config_1.default.existingOutput) {
    console.log('Parsing using existing output file...');
    mainPromises[0] = readFile(config_1.default.existingOutput, 'utf-8').then(JSON.parse);
}
else if (config_1.default.source) {
    console.log('Parsing using TypeDoc...');
    var files_1 = [];
    for (var _i = 0, _a = config_1.default.source; _i < _a.length; _i++) {
        var dir = _a[_i];
        files_1.push(dir + "/*.ts", dir + "/**/*.ts");
    }
    mainPromises[0] = new Promise(function (res, rej) {
        ...
        var writeResult = app.generateJson(files_1, filePath);
        if (!writeResult)
